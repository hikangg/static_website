(function($){
	...
	$(document).ready( function(){
		...
		$('.left-wrap .bg').backstretch('theme/images/bg.jpg');
			...
			submitHandler: function(form){
				...
				$.post($(form).attr('action'), $(form).serialize(), function(data){
					$('.newsletter').find('.spinner').animate({opacity: 0}, function(){
						$(this).hide();
						$('.newsletter').find('.success').show().html('<i class="icon ion-ios7-checkmark-outline"></i> Thank you for your message. We will get back to you within one business day.').animate({opacity: 1});
					});
				});
				return false;
			}
	...
	function jgtGoogleMap(){
		...
		var mapLat = 38.874526,
			mapLong = -77.085400,
			mapInfo = '<div id="content">Abnoba LLC</div>';
